...
import { validateProductAllocations } from "@shared/clientProductCollection";
import { invoiceReceiptValues } from "@shared/invoiceReceiptTotals";
...
export const billingItemInput = z.object({ itemName: z.string().trim().min(1).max(255), description: z.string().trim().max(500).optional(), quantity: z.coerce.number().positive(), unitPrice: z.coerce.number().nonnegative(), productId: z.coerce.number().int().positive().optional(), collectionAmount: z.coerce.number().positive().optional() });
export const invoiceInput = z.object({ projectId: z.coerce.number().int().positive().optional(), clientId: z.string().trim().max(64).optional(), clientName: z.string().trim().min(1).max(255), clientAddress: z.string().trim().min(1), clientContact: z.string().trim().max(64).optional(), clientEmail: z.string().trim().email().max(320).optional(), clientGst: z.string().trim().max(32).optional(), invoiceDate: z.string().min(1), dueDate: z.string().min(1), items: z.array(billingItemInput).min(1), gstRate: z.coerce.number().nonnegative().max(100).default(18), gstMode: z.enum(["inclusive", "exclusive"]).default("exclusive"), notes: z.string().trim().max(2000).optional(), invoiceNumber: z.string().trim().max(32).optional() });
...
export const appRouter = router({
  dashboard: router({
    clientPayments: protectedProcedure.query(({ ctx }) => getClientPaymentSummaryForOwner(ctx.user.id)),
  }),
  invoices: router({
    settings: router({
  ...
  receipts: router({
    ...
    create: protectedProcedure.input(receiptInput).mutation(async ({ ctx, input }) => {
      ...
      const sourceInvoice = input.invoiceId ? (await listInvoicesForOwner(ctx.user.id)).find(row => Number(row.id) === Number(input.invoiceId)) : null;
      if (input.invoiceId && !sourceInvoice) throw new TRPCError({ code: "NOT_FOUND", message: "Source invoice not found." });
      let sourceItems: any[] = items;
      if (sourceInvoice?.itemsJson) {
        try { sourceItems = JSON.parse(sourceInvoice.itemsJson); } catch { sourceItems = items; }
      }
      const effectiveItems = sourceInvoice ? sourceItems : items;
      const allocations = effectiveItems.filter((item: any) => item.productId && item.collectionAmount).map((item: any) => ({ productId: item.productId!, amount: item.collectionAmount! }));
      if (input.clientId && allocations.length) {
        const products = await listClientProductsForOwner(ctx.user.id, input.clientId);
        validateProductAllocations(products as any, allocations);
      }
      const calculatedTotals = calculateQuotationTotals(effectiveItems as QuotationItem[], input.gstRate, input.gstMode);
      const exactValues = invoiceReceiptValues(sourceInvoice ?? {}, { subtotal: calculatedTotals.subtotal.toFixed(2), gstRate: input.gstRate.toFixed(2), gstMode: input.gstMode, gstAmount: calculatedTotals.gstAmount.toFixed(2), grandTotal: calculatedTotals.grandTotal.toFixed(2) });
      const { subtotal, gstRate, gstMode, gstAmount, grandTotal } = exactValues;
      const receipt = await createReceiptForOwner(ctx.user.id, { ...fields, itemsJson: JSON.stringify(effectiveItems), subtotal, gstRate, gstMode, gstAmount, grandTotal, amount: grandTotal, receivedFor: effectiveItems.map((item: any) => item.itemName).join(", ").slice(0, 255), companyGst: sourceInvoice?.companyGst || settings.companyGst, companyAddress: sourceInvoice?.companyAddress || settings.companyAddress, terms: sourceInvoice?.terms || settings.terms, accountCompanyName: sourceInvoice?.accountCompanyName || settings.accountCompanyName, accountNumber: sourceInvoice?.accountNumber || settings.accountNumber, accountIfsc: sourceInvoice?.accountIfsc || settings.accountIfsc, accountBranch: sourceInvoice?.accountBranch || settings.accountBranch, logoUrl: sourceInvoice?.logoUrl || settings.logoUrl, logoKey: sourceInvoice?.logoKey || settings.logoKey, signatureUrl: sourceInvoice?.signatureUrl || settings.signatureUrl, signatureKey: sourceInvoice?.signatureKey || settings.signatureKey, footerCompanyName: settings.footerCompanyName, footerMessage: settings.footerMessage, qrLabel: settings.qrLabel });
      if (input.invoiceId) await updateInvoiceStatusForOwner(ctx.user.id, input.invoiceId, "Paid");
      if (input.clientId && allocations.length) await applyClientProductCollectionsForOwner(ctx.user.id, { clientId: input.clientId, allocations, paymentDate: input.paymentDate, paymentMode: input.paymentMode, transactionReference: input.transactionReference });
      return receipt;
    }),
  ...
  clients: router({
    list: protectedProcedure.input(z.object({ page: z.number().int().min(1).default(1), pageSize: z.number().int().min(10).max(1000).default(25), search: z.string().trim().max(100).optional(), instituteType: z.enum(["School", "College", "Academy"]).optional(), clientStatus: z.enum(["Active", "Inactive", "Hold", "Cancelled", "Renewal", "Extended", "Closed", "Ready to Expire", "Expired"]).optional(), startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(), endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(), branchCoverage: z.enum(["individual", "multiple"]).optional(), minValue: z.number().nonnegative().optional(), maxValue: z.number().nonnegative().optional(), sessionMode: z.enum(["all", "single"]).optional(), currentSession: z.string().regex(/^\d{4}-\d{4}$/).optional(), projectId: z.number().int().positive().optional() })).query(async ({ ctx, input }) => listApprovedClientsForOwner(ctx.user.id, { ...input, ...(input.sessionMode ? {} : await getSessionSettings(ctx.user.id)) })),
    create: protectedProcedure.input(z.object({ projectId: z.number().int().positive(), clientName: z.string().trim().min(1).max(255), clientOwnerName: z.string().trim().min(1).max(255), instituteType: z.enum(["School", "College", "Academy"]).default("School"), branchCoverage: z.enum(["individual", "multiple"]).default("individual"), branchCount: z.number().int().positive().max(100000).default(1), contactNumber: z.string().trim().min(1).max(64), email: z.string().trim().email().max(320), address: z.string().trim().min(1), price: z.number().nonnegative(), noOfStudents: z.number().int().nonnegative().default(0), pricingMode: z.enum(["perStudent", "package"]).default("package"), perStudentPrice: z.number().nonnegative().optional().nullable(), packagePrice: z.number().nonnegative().optional().nullable(), gstRate: z.number().nonnegative().max(100).default(18), gstMode: z.enum(["inclusive", "exclusive"]).default("exclusive"), startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), session: z.string().regex(/^\d{4}-\d{4}$/), description: z.string().max(5000).optional().nullable(), logoUrl: z.string().optional().nullable(), logoKey: z.string().optional().nullable(), status: z.enum(["Active", "Inactive", "Hold", "Cancelled", "Renewal", "Extended", "Closed"]).default("Active") })).mutation(({ ctx, input }) => { const pricing = calculateErpPricing({ mode: input.pricingMode, noOfStudents: input.noOfStudents, perStudentPrice: input.perStudentPrice, packagePrice: input.packagePrice, fallbackPrice: input.price, gstRate: input.gstRate, gstMode: input.gstMode }); return createClientForOwner(ctx.user.id, input.projectId, { ...input, noOfStudents: pricing.noOfStudents, pricingMode: input.pricingMode, perStudentPrice: pricing.perStudentPrice === null ? null : pricing.perStudentPrice.toFixed(2), packagePrice: pricing.packagePrice === null ? null : pricing.packagePrice.toFixed(2), noOfYearPlan: 1, price: pricing.entered.toFixed(2), gstRate: input.gstRate.toFixed(2), gstMode: input.gstMode, gstAmount: pricing.gstAmount.toFixed(2), totalPrice: pricing.totalPrice.toFixed(2) }); }),
    update: protectedProcedure.input(z.object({ id: z.number().int(), clientName: z.string().trim().min(1).max(255), clientOwnerName: z.string().trim().min(1).max(255), instituteType: z.enum(["School", "College", "Academy"]).default("School"), branchCoverage: z.enum(["individual", "multiple"]).default("individual"), branchCount: z.number().int().positive().max(100000).default(1), contactNumber: z.string().trim().min(1).max(64), email: z.string().trim().email().max(320), address: z.string().trim().min(1), price: z.number().nonnegative(), noOfStudents: z.number().int().nonnegative().default(0), pricingMode: z.enum(["perStudent", "package"]).default("package"), perStudentPrice: z.number().nonnegative().optional().nullable(), packagePrice: z.number().nonnegative().optional().nullable(), gstRate: z.number().nonnegative().max(100).default(18), gstMode: z.enum(["inclusive", "exclusive"]).default("exclusive"), startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), session: z.string().regex(/^\d{4}-\d{4}$/), description: z.string().max(5000).optional().nullable(), status: z.enum(["Active", "Inactive", "Hold", "Cancelled", "Renewal", "Extended", "Closed"]).default("Active") })).mutation(async ({ ctx, input }) => { const { id, ...fields } = input; if (id > 0) { const { price: _price, status, gstRate, ...agreementFields } = fields; return updateAgreementByIdForOwner(ctx.user.id, id, { ...agreementFields, perStudentPrice: agreementFields.perStudentPrice == null ? null : agreementFields.perStudentPrice.toFixed(2), packagePrice: agreementFields.packagePrice == null ? null : agreementFields.packagePrice.toFixed(2), gstRate: gstRate.toFixed(2), clientStatus: status }); } const pricing = calculateErpPricing({ mode: fields.pricingMode, noOfStudents: fields.noOfStudents, perStudentPrice: fields.perStudentPrice, packagePrice: fields.packagePrice, fallbackPrice: fields.price, gstRate: fields.gstRate, gstMode: fields.gstMode }); return updateClientForOwner(ctx.user.id, Math.abs(id), { ...fields, noOfStudents: pricing.noOfStudents, pricingMode: fields.pricingMode, perStudentPrice: pricing.perStudentPrice === null ? null : pricing.perStudentPrice.toFixed(2), packagePrice: pricing.packagePrice === null ? null : pricing.packagePrice.toFixed(2), noOfYearPlan: 1, price: pricing.entered.toFixed(2), gstRate: fields.gstRate.toFixed(2), gstMode: fields.gstMode, gstAmount: pricing.gstAmount.toFixed(2), totalPrice: pricing.totalPrice.toFixed(2) }); }),
    updateStatus: protectedProcedure.input(z.object({ id: z.number().int(), status: z.enum(["Active", "Inactive", "Hold", "Cancelled", "Renewal", "Extended", "Closed"]) })).mutation(async ({ ctx, input }) => { if (input.id < 0) await updateClientForOwner(ctx.user.id, Math.abs(input.id), { status: input.status }); else await updateAgreementClientStatus(ctx.user.id, input.id, input.status); return { success: true }; }),
    renewDirect: protectedProcedure.input(z.object({ clientId: z.number().int().positive(), renewalType: z.enum(["continuous", "sixMonths", "oneYear"]), startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(), endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional() })).mutation(({ ctx, input }) => renewClientForOwner(ctx.user.id, input.clientId, input.renewalType, { startDate: input.startDate, endDate: input.endDate })),
    paymentPlan: protectedProcedure.input(z.object({ clientId: z.string().trim().min(1).max(64) })).query(({ ctx, input }) => getPaymentPlanForOwner(ctx.user.id, input.clientId)),
    savePaymentPlan: protectedProcedure.input(z.object({ clientId: z.string().trim().min(1).max(64), projectId: z.number().int().positive().nullable().optional(), paymentCycle: z.enum(["single", "terms", "installments", "months"]), totalAmount: z.number().nonnegative(), initialPayment: z.number().nonnegative(), terms: z.array(z.object({ label: z.string().trim().min(1).max(128), dueDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), amount: z.number().nonnegative() })).max(100) })).mutation(({ ctx, input }) => savePaymentPlanForOwner(ctx.user.id, { ...input, totalAmount: input.totalAmount.toFixed(2), initialPayment: input.initialPayment.toFixed(2), terms: input.terms.map((term) => ({ ...term, amount: term.amount.toFixed(2) })) })),
    products: protectedProcedure.input(z.object({ clientId: z.string().trim().min(1).max(64) })).query(({ ctx, input }) => listClientProductsForOwner(ctx.user.id, input.clientId)),
    saveProducts: protectedProcedure.input(z.object({ clientId: z.string().trim().min(1).max(64), projectId: z.number().int().positive().nullable().optional(), products: z.array(z.object({ productName: z.string().trim().min(1).max(255), description: z.string().trim().max(2000).optional().nullable(), quantity: z.number().positive(), unitPrice: z.number().nonnegative(), gstRate: z.number().nonnegative().max(100), gstMode: z.enum(["inclusive", "exclusive"]), paidAmount: z.number().nonnegative(), dueDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().nullable(), paymentDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().nullable(), paymentMode: z.string().trim().max(64).optional().nullable(), transactionReference: z.string().trim().max(128).optional().nullable() })).max(100) })).mutation(({ ctx, input }) => saveClientProductsForOwner(ctx.user.id, input)),
  }),
  agreements: router({

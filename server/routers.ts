...
export const appRouter = router({
  ...
  receipts: router({
    ...
    create: protectedProcedure.input(receiptInput).mutation(async ({ ctx, input }) => {
      ...
      const allocations = items.filter(item => item.productId && item.collectionAmount).map(item => ({ productId: item.productId!, amount: item.collectionAmount! }));
      ...
      const receipt = await createReceiptForOwner(ctx.user.id, { ...fields, itemsJson: JSON.stringify(items), subtotal: subtotal.toFixed(2), gstRate: input.gstRate.toFixed(2), gstMode: input.gstMode, gstAmount: gstAmount.toFixed(2), grandTotal: grandTotal.toFixed(2), amount: grandTotal.toFixed(2), receivedFor: items.map(item => item.itemName).join(", ").slice(0, 255), companyGst: settings.companyGst, companyAddress: settings.companyAddress, terms: settings.terms, accountCompanyName: settings.accountCompanyName, accountNumber: settings.accountNumber, accountIfsc: settings.accountIfsc, accountBranch: settings.accountBranch, logoUrl: settings.logoUrl, logoKey: settings.logoKey, signatureUrl: settings.signatureUrl, signatureKey: settings.signatureKey, footerCompanyName: settings.footerCompanyName, footerMessage: settings.footerMessage, qrLabel: settings.qrLabel });

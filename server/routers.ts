...
export const receiptInput = z.object({ clientName: z.string().trim().min(1).max(255), clientAddress: z.string().trim().min(1), clientContact: z.string().trim().max(64).optional(), clientEmail: z.string().trim().email().max(320).optional(), receiptDate: z.string().min(1), paymentDate: z.string().min(1), amount: z.coerce.number().nonnegative(), paymentMode: z.enum(["Cash", "UPI", "Bank Transfer", "Card", "Cheque", "Other"]), transactionReference: z.string().trim().max(128).optional(), receivedFor: z.string().trim().min(1).max(255), notes: z.string().trim().max(2000).optional(), receiptNumber: z.string().trim().max(32).optional() });
export const receiptSettingsInput = z.object({ companyGst: z.string().trim().min(1).max(32), companyAddress: z.string().trim().min(1), receiptPrefix: z.string().trim().min(1).max(24), receiptNumberStart: z.coerce.number().int().positive(), terms: z.string().max(4000), accountCompanyName: z.string().trim().max(255).optional(), accountNumber: z.string().trim().max(128).optional(), accountIfsc: z.string().trim().max(64).optional(), accountBranch: z.string().trim().max(255).optional(), footerCompanyName: z.string().trim().max(255).optional(), footerMessage: z.string().trim().max(255).optional(), qrLabel: z.string().trim().max(64).optional() });

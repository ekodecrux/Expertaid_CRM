...
export const invoiceInput = z.object({ clientId: z.string().trim().max(64).optional(), clientName: z.string().trim().min(1).max(255), clientAddress: z.string().trim().min(1), clientContact: z.string().trim().max(64).optional(), clientEmail: z.string().trim().email().max(320).optional(), clientGst: z.string().trim().max(32).optional(), invoiceDate: z.string().min(1), dueDate: z.string().min(1), items: z.array(billingItemInput).min(1), gstRate: z.coerce.number().nonnegative().max(100).default(18), gstMode: z.enum(["inclusive", "exclusive"]).default("exclusive"), notes: z.string().trim().max(2000).optional(), invoiceNumber: z.string().trim().max(32).optional() });

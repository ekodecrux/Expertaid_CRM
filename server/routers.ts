...
export const appRouter = router({
  ...
  clients: router({
    ...
    create: protectedProcedure.input(z.object({ projectId: z.number().int().positive(), clientName: z.string().trim().min(1).max(255), clientOwnerName: z.string().trim().min(1).max(255), instituteType: z.enum(["School", "College", "Academy"]).default("School"), branchCoverage: z.enum(["individual", "multiple"]).default("individual"), branchCount: z.number().int().positive().max(100000).default(1), contactNumber: z.string().trim().min(1).max(64), email: z.string().trim().email().max(320), address: z.string().trim().min(1), price: z.number().nonnegative(), gstRate: z.number().nonnegative().max(100).default(18), gstMode: z.enum(["inclusive", "exclusive"]).default("exclusive"), startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), session: z.string().regex(/^\d{4}-\d{4}$/), description: z.string().max(5000).optional().nullable(), logoUrl: z.string().optional().nullable(), logoKey: z.string().optional().nullable(), status: z.enum(["Active", "Inactive", "Hold", "Cancelled", "Renewal", "Extended", "Closed"]).default("Active") })).mutation(({ ctx, input }) => { const entered = input.price; const divisor = 1 + input.gstRate / 100; const gstAmount = input.gstMode === "inclusive" ? entered - entered / divisor : entered * input.gstRate / 100; const totalPrice = input.gstMode === "inclusive" ? entered : entered + gstAmount; return createClientForOwner(ctx.user.id, input.projectId, { ...input, noOfStudents: 0, pricingMode: "package", perStudentPrice: null, packagePrice: entered.toFixed(2), noOfYearPlan: 1, price: entered.toFixed(2), gstRate: input.gstRate.toFixed(2), gstMode: input.gstMode, gstAmount: gstAmount.toFixed(2), totalPrice: totalPrice.toFixed(2)       }); }),
    update: protectedProcedure.input(z.object({ id: z.number().int(), clientName: z.string().trim().min(1).max(255), clientOwnerName: z.string().trim().min(1).max(255), instituteType: z.enum(["School", "College", "Academy"]).default("School"), branchCoverage: z.enum(["individual", "multiple"]).default("individual"), branchCount: z.number().int().positive().max(100000).default(1), contactNumber: z.string().trim().min(1).max(64), email: z.string().trim().email().max(320), address: z.string().trim().min(1), price: z.number().nonnegative(), gstRate: z.number().nonnegative().max(100).default(18), gstMode: z.enum(["inclusive", "exclusive"]).default("exclusive"), startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), session: z.string().regex(/^\d{4}-\d{4}$/), description: z.string().max(5000).optional().nullable(), status: z.enum(["Active", "Inactive", "Hold", "Cancelled", "Renewal", "Extended", "Closed"]).default("Active") })).mutation(async ({ ctx, input }) => { const { id, ...fields } = input; if (id > 0) { const { price: _price, status, gstRate, ...agreementFields } = fields; return updateAgreementByIdForOwner(ctx.user.id, id, { ...agreementFields, gstRate: gstRate.toFixed(2), clientStatus: status }); } const entered = fields.price; const divisor = 1 + fields.gstRate / 100; const gstAmount = fields.gstMode === "inclusive" ? entered - entered / divisor : entered * fields.gstRate / 100; const totalPrice = fields.gstMode === "inclusive" ? entered : entered + gstAmount; return updateClientForOwner(ctx.user.id, Math.abs(id), { ...fields, noOfStudents: 0, pricingMode: "package", perStudentPrice: null, packagePrice: entered.toFixed(2), noOfYearPlan: 1, price: entered.toFixed(2), gstRate: fields.gstRate.toFixed(2), gstMode: fields.gstMode, gstAmount: gstAmount.toFixed(2), totalPrice: totalPrice.toFixed(2) }); }),

...
export const appRouter = router({
  ...
  clients: router({
    ...
    create: protectedProcedure.input(z.object({ projectId: z.number().int().positive(), clientName: z.string().trim().min(1).max(255), clientOwnerName: z.string().trim().min(1).max(255), instituteType: z.enum(["School", "College", "Academy"]).default("School"), branchCoverage: z.enum(["individual", "multiple"]).default("individual"), branchCount: z.number().int().positive().max(100000).default(1), contactNumber: z.string().trim().min(1).max(64), email: z.string().trim().email().max(320), address: z.string().trim().min(1), noOfStudents: z.number().int().nonnegative().default(0), pricingMode: z.enum(["perStudent", "package"]).default("perStudent"), perStudentPrice: z.string().optional().nullable(), packagePrice: z.string().optional().nullable(), noOfYearPlan: z.number().int().positive().max(100).default(1), startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), session: z.string().regex(/^\d{4}-\d{4}$/), totalPrice: z.string().default("0.00"), description: z.string().max(5000).optional().nullable(), logoUrl: z.string().optional().nullable(), logoKey: z.string().optional().nullable() })).mutation(({ ctx, input }) => createClientForOwner(ctx.user.id, input.projectId, input)),
  }),
  agreements: router({

...
export const appRouter = router({
  ...
  clients: router({
    ...
    update: protectedProcedure.input(z.object({ id: z.number().int().positive(), clientName: z.string().trim().min(1).max(255), clientOwnerName: z.string().trim().min(1).max(255), instituteType: z.enum(["School", "College", "Academy"]).default("School"), branchCoverage: z.enum(["individual", "multiple"]).default("individual"), branchCount: z.number().int().positive().max(100000).default(1), contactNumber: z.string().trim().min(1).max(64), email: z.string().trim().email().max(320), address: z.string().trim().min(1), price: z.number().nonnegative(), gstRate: z.number().nonnegative().max(100).default(18), gstMode: z.enum(["inclusive", "exclusive"]).default("exclusive"), startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), session: z.string().regex(/^\d{4}-\d{4}$/), description: z.string().max(5000).optional().nullable(), status: z.enum(["Active", "Inactive", "Hold", "Cancelled", "Renewal", "Extended", "Closed"]).default("Active") })).mutation(({ ctx, input }) => { const { id, ...fields } = input; const entered = fields.price; const divisor = 1 + fields.gstRate / 100; const gstAmount = fields.gstMode === "inclusive" ? entered - entered / divisor : entered * fields.gstRate / 100; const totalPrice = fields.gstMode === "inclusive" ? entered : entered + gstAmount; return updateClientForOwner(ctx.user.id, id, { ...fields, noOfStudents: 0, pricingMode: "package", perStudentPrice: null, packagePrice: entered.toFixed(2), noOfYearPlan: 1, price: entered.toFixed(2), gstRate: fields.gstRate.toFixed(2), gstMode: fields.gstMode, gstAmount: gstAmount.toFixed(2), totalPrice: totalPrice.toFixed(2) }); }),

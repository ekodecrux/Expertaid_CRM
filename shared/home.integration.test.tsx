...
vi.mock("@/lib/trpc", () => ({ trpc: { auth: { me: { useQuery: () => authQueryState }, logout: { useMutation: () => mutationResult } }, branding: { get: { useQuery: () => ({ data: { companyLogoUrl: "/logo.png", companyName: "Expertaid Technologies", serviceCaption: "ERP Solutions", footerCompanyName: "Expertaid Technologies Pvt Ltd" } }) } }, session: { get: { useQuery: () => ({ data: { sessionMode: "single", currentSession: "2026-2027" } }) }, update: { useMutation: () => mutationResult } }, agreements: { sessions: { useQuery: () => ({ data: ["2026-2027"] }) }, list: { useQuery: () => listResult }, create: { useMutation: () => mutationResult }, update: { useMutation: () => mutationResult } }, useUtils: () => ({ auth: { me: { setData: vi.fn(), invalidate: vi.fn() } }, branding: { get: { invalidate: vi.fn() } }, session: { get: { invalidate: vi.fn() } } }) } }));

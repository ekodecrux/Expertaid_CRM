  ...
  return <div className="agreement-page min-h-screen bg-[#f7f9fd] px-3 py-4 font-sans text-[#172033] sm:px-6 lg:px-8 print:bg-white print:p-0">
    <div className="agreement-sheet mx-auto max-w-5xl overflow-hidden bg-white shadow-[0_22px_70px_rgba(30,45,80,0.1)] print:max-w-none print:shadow-none">
      ...
      <main className="px-5 pb-6 pt-5 sm:px-10 sm:pb-8 sm:pt-7 print:px-8 print:pb-4 print:pt-5">
        <header className="relative border-b border-slate-100 pb-5 print:pb-3"><Button type="button" variant="outline" size="icon" aria-label="Cancel and close agreement" title="Cancel and close" className="absolute right-0 top-0 z-10 h-9 w-9 rounded-full border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-800 print:hidden" onClick={() => { if (window.history.length > 1) window.history.back(); else window.location.assign("/"); }}><X className="h-4 w-4" /></Button>
          <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex items-center gap-2 self-end pr-12 print:hidden sm:self-auto"><Badge className={agreement.status === "Approved" ? "bg-emerald-100 text-emerald-700" : agreement.status === "Rejected" ? "bg-rose-100 text-rose-700" : "bg-amber-100 text-amber-700"}>{agreement.status}</Badge><Button variant="outline" size="sm" onClick={() => window.print()}><Printer className="mr-2 h-4 w-4" />Print</Button></div>
          </div>

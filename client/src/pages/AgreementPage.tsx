...
export default function AgreementPage() {
  ...
  return <div className="agreement-page min-h-screen bg-[#f5f7fb] px-4 py-5 font-sans text-[#172033] sm:px-8 lg:py-8 print:min-h-0 print:bg-white print:p-0">
    <div className="mx-auto max-w-4xl print:max-w-none">
      <header className="mb-4 border-b border-slate-200 pb-4 print:mb-2 print:pb-2">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex min-w-0 items-center gap-3"><img src="/manus-storage/EXPLOGO2024_3ab64898.png" alt="Expertaid Technologies Pvt. Ltd." className="h-10 w-auto max-w-[190px] object-contain sm:h-12" /><div className="min-w-0"><p className="text-sm font-semibold tracking-tight text-[#172033] sm:text-base">Expertaid Technologies Pvt. Ltd.</p><p className="mt-0.5 text-[10px] font-medium uppercase tracking-[0.16em] text-slate-400">ERP solutions & agreement management</p></div></div>
          <div className="flex shrink-0 items-center gap-2 self-end print:hidden sm:self-auto"><Badge className={agreement.status === "Approved" ? "bg-emerald-100 text-emerald-700" : agreement.status === "Rejected" ? "bg-rose-100 text-rose-700" : "bg-amber-100 text-amber-700"}>{agreement.status}</Badge><Button variant="outline" size="sm" onClick={() => window.print()}><Printer className="mr-2 h-4 w-4" />Print</Button></div>
        </div>
        <div className="mt-3 flex items-end justify-between gap-4 print:mt-2"><div><h1 className="font-serif text-2xl tracking-tight text-[#172033] sm:text-3xl">Client Agreement</h1><p className="mt-1 text-xs text-slate-500 sm:text-sm">Please review the agreement details carefully before accepting.</p></div></div>
      </header>

      <Card className="agreement-document overflow-hidden border-0 shadow-[0_20px_70px_rgba(30,45,80,0.09)] print:shadow-none print:ring-1 print:ring-slate-200">
        <div className="h-2 bg-[#3157d5]" />
        <CardContent className="space-y-5 p-5 sm:p-8 print:space-y-3 print:p-5">
          <section className="flex flex-col justify-between gap-4 border-b border-slate-100 pb-5 sm:flex-row"><div className="flex items-center gap-3"><img src={agreement.logoUrl || "/manus-storage/EXPLOGO2024_3ab64898.png"} alt={`${agreement.clientName} logo`} className="h-14 w-32 shrink-0 rounded-xl border border-slate-100 bg-white object-contain p-2 print:h-10 print:w-28" /><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Prepared for</p><h2 className="mt-1 text-xl font-semibold sm:text-2xl">{agreement.clientName}</h2><p className="mt-1 text-sm text-slate-500">Attention: {agreement.clientOwnerName}</p></div></div><div className="text-left sm:text-right"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Agreement date</p><p className="mt-2 font-medium">{new Date(agreement.createdAt).toLocaleDateString("en-IN", { dateStyle: "long" })}</p><p className="mt-1 text-xs text-slate-500">Reference #{agreement.id.toString().padStart(5, "0")}</p></div></section>
          <section><div className="mb-3 flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-[#3157d5]" /><h2 className="font-semibold">Client & plan information</h2></div><div className="grid gap-3 sm:grid-cols-2"><Info label="Contact number" value={agreement.contactNumber} /><Info label="Email address" value={agreement.email} /><Info label="Institute type" value={agreement.instituteType ?? "School"} /><Info label="Branch coverage" value={agreement.branchCoverage === "multiple" ? `${agreement.branchCount} branches` : "Individual branch"} /><Info label="Address" value={agreement.address} wide /><Info label="Number of students" value={agreement.noOfStudents.toLocaleString("en-IN")} /><Info label={agreement.pricingMode === "package" ? "Package price" : "Per student price"} value={formatMoney(agreement.pricingMode === "package" ? (agreement.packagePrice ?? 0) : (agreement.perStudentPrice ?? 0))} /><Info label="Plan duration" value={`${agreement.noOfYearPlan} ${agreement.noOfYearPlan === 1 ? "year" : "years"}`} /><Info label="Plan period" value={`${agreement.startDate} — ${agreement.endDate}`} /></div></section>
          <section className="rounded-2xl bg-[#f3f6ff] p-4 sm:p-5 print:p-3"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#3157d5]">Agreement value</p><p className="mt-2 text-3xl font-semibold tracking-tight text-[#172033]">{formatMoney(agreement.totalPrice)}</p><p className="mt-1 text-sm text-slate-500">Calculated from the selected pricing method and plan duration.</p></div><div className="hidden rounded-xl bg-white p-3 sm:block"><FileCheck2 className="h-6 w-6 text-[#3157d5]" /></div></div></section>

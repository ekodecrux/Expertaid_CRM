...
export default function Settings() {
  ...
  return <DashboardLayout><div className="min-h-[calc(100vh-2rem)] bg-[#f7f8fc] -m-3 p-4 text-[#172033] sm:-m-4 sm:p-8 lg:p-10"><div className="mx-auto max-w-5xl">
    ...
    <form onSubmit={submitSession} className="mt-6"><Card className="border-0 shadow-[0_18px_60px_rgba(30,45,80,0.07)]"><CardHeader><CardTitle className="font-serif text-2xl">Current session</CardTitle><CardDescription>Choose whether the workspace shows all sessions or one academic/business session.</CardDescription></CardHeader><CardContent className="grid gap-5 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="sessionMode">Session view</Label><select id="sessionMode" value={sessionMode} onChange={e => setSessionMode(e.target.value as "all" | "single")} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground"><option value="all">All sessions</option><option value="single">Single session</option></select></div><div className="space-y-2"><Label htmlFor="currentSession">Current session</Label><select id="currentSession" value={currentSession} onChange={e => setCurrentSession(e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground"><option value={currentSession}>{currentSession}</option>{(sessions.data ?? []).filter((session) => session.sessionLabel !== currentSession).map((session) => <option key={session.sessionLabel} value={session.sessionLabel}>{session.sessionLabel} · {session.startDate} – {session.endDate}</option>)}</select><p className="text-xs text-slate-400">Select one of your managed sessions.</p></div><div className="flex justify-end sm:col-span-2"><Button type="submit" className="bg-[#3157d5] text-white hover:bg-[#2748bd]" disabled={updateSession.isPending || sessionSettings.isLoading}>{updateSession.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Save session settings</Button></div></CardContent></Card></form>
    <form onSubmit={submitNewSession} className="mt-6"><Card className="border-0 shadow-[0_18px_60px_rgba(30,45,80,0.07)]"><CardHeader><CardTitle className="font-serif text-2xl">Manage sessions</CardTitle><CardDescription>Add previous, current, or future sessions with their exact start and end dates.</CardDescription></CardHeader><CardContent><div className="mb-5 space-y-3">{(sessions.data ?? []).map((session) => <div key={session.sessionLabel} className="flex flex-col gap-1 rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 text-sm sm:flex-row sm:items-center sm:justify-between"><span className="font-semibold text-slate-700">{session.sessionLabel}</span><span className="text-xs text-slate-500">{session.startDate} – {session.endDate}</span></div>)}</div><div className="grid gap-4 sm:grid-cols-3"><div className="space-y-2"><Label htmlFor="newSessionLabel">Session label</Label><Input id="newSessionLabel" value={newSessionLabel} onChange={e => setNewSessionLabel(e.target.value)} pattern="\\d{4}-\\d{4}" placeholder="2027-2028" required /></div><div className="space-y-2"><Label htmlFor="newSessionStart">Start date</Label><Input id="newSessionStart" type="date" value={newSessionStart} onChange={e => setNewSessionStart(e.target.value)} required /></div><div className="space-y-2"><Label htmlFor="newSessionEnd">End date</Label><Input id="newSessionEnd" type="date" value={newSessionEnd} onChange={e => setNewSessionEnd(e.target.value)} required /></div></div><div className="mt-5 flex justify-end"><Button type="submit" className="bg-[#3157d5] text-white hover:bg-[#2748bd]" disabled={createSession.isPending}>{createSession.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Add session</Button></div></CardContent></Card></form>

...
function MetricCard({ label, value, icon: Icon, tint, foot }: { label: string; value: string; icon: typeof UsersRound; tint: string; foot?: string }) {
  return <Card className="h-fit self-start rounded-xl border border-slate-100 shadow-[0_8px_24px_rgba(30,45,80,0.05)]"><CardContent className="p-2.5"><div className="flex items-start justify-between gap-2"><div><p className="text-[9px] font-bold uppercase tracking-[0.12em] text-slate-400">{label}</p><p className="mt-1.5 text-xl font-extrabold tracking-tight text-slate-900">{value}</p></div><span className={`flex h-8 w-8 items-center justify-center rounded-lg ${tint}`}><Icon className="h-4 w-4" /></span></div>{foot && <p className="mt-1.5 text-[10px] leading-4 text-slate-500">{foot}</p>}</CardContent></Card>;
}

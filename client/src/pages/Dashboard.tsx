...
export default function Dashboard() {
  ...
  const computed = useMemo(() => {
    ...
    const monthlyCollections = monthLabels.map((label, index) => ({ label, amount: validReceipts.filter(row => { const date = new Date(String(row.paymentDate ?? row.createdAt)); return date.getFullYear() === year && date.getMonth() === index; }).reduce((sum, row) => sum + Number(row.amount ?? row.grandTotal ?? 0), 0) }));
    const clientGrowth = monthLabels.map((label, index) => ({ label, count: clientRows.filter(row => { const date = new Date(String(row.createdAt ?? row.startDate)); return date.getFullYear() === year && date.getMonth() === index; }).length }));
    ...
    return { invoiceValue, receiptValue, totalClientValue, todayCollected, monthlyCollected, yearCollected, monthlyCollections, clientGrowth, dueClientPayments, aging, pendingClientPayments: dueClientPayments.length, dueTotal: clientDueTotal };

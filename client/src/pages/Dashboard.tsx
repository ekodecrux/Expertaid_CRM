...
export default function Dashboard() {
  ...
  const computed = useMemo(() => {
    ...
    return { invoiceValue, receiptValue, totalClientValue, todayCollected, monthlyCollected, yearCollected, dueClientPayments, aging, pendingClientPayments: dueClientPayments.length, dueTotal: clientDueTotal };
  ...
  return <DashboardLayout><main className="min-h-full bg-[#f7f8fc] p-3 sm:p-5"><div className="mx-auto max-w-[1450px]">
    ...
    <div className="mt-3 grid gap-3 xl:grid-cols-[0.95fr_1.35fr]"><Card className="border border-slate-100 shadow-sm"><CardHeader className="px-4 pb-2 pt-3"><PanelTitle title="Due Payments" action="View clients" href="/clients" /></CardHeader><CardContent className="grid grid-cols-[120px_1fr] items-center gap-4 px-4 pb-4"><div className="relative mx-auto flex h-28 w-28 items-center justify-center rounded-full" style={{ background: donutStyle }}><div className="flex h-[68px] w-[68px] flex-col items-center justify-center rounded-full bg-white"><span className="text-lg font-bold text-slate-900">{computed.dueClientPayments.length}</span><span className="text-[9px] text-slate-500">Client payments</span></div></div><div className="space-y-2">{computed.aging.map(bucket => <div key={bucket.label} className="flex items-center justify-between gap-2 text-[10px]"><span className="flex items-center gap-1.5 text-slate-600"><i className={`h-2 w-2 rounded-full ${bucket.color}`} />{bucket.label}</span><span className="font-semibold text-slate-800">{money(bucket.amount)} ({bucket.count})</span></div>)}</div></CardContent></Card>
      <Card className="border border-slate-100 shadow-sm"><CardHeader className="flex-row items-center justify-between px-4 pb-2 pt-3"><PanelTitle title="Client Payment Aging" /><select className="h-7 rounded-md border border-slate-200 bg-white px-2 text-[10px] text-slate-600"><option>This Year</option></select></CardHeader><CardContent className="px-4 pb-4"><div className="flex h-32 items-end justify-around gap-3 border-b border-l border-slate-200 px-3 pb-0 pt-3">{computed.aging.map(bucket => <div key={bucket.label} className="flex h-full flex-1 flex-col items-center justify-end gap-1"><span className="text-[9px] font-semibold text-slate-600">{money(bucket.amount)}</span><div className={`w-full max-w-[48px] rounded-t-md ${bucket.color}`} style={{ height: `${Math.max(5, (bucket.amount / maxAging) * 76)}%` }} /><span className="mb-1 text-[9px] text-slate-500">{bucket.label.replace(" Days", "")}</span></div>)}</div></CardContent></Card></div>

...
export default function Dashboard() {
  ...
  const agreements = trpc.agreements.list.useQuery(sessionMode === "all" ? undefined : { session: currentSession });
  const clients = trpc.clients.list.useQuery({ page: 1, pageSize: 1000, sessionMode, currentSession: sessionMode === "all" ? undefined : currentSession });
  ...
  const computed = useMemo(() => {
    ...
    const validReceipts = filterReceiptsForDashboardSession(receiptRows, sessionClientIds);
    ...
    const sessionReceipts = sessionMode === "all" ? validReceipts : validReceipts.filter(row => { const date = String(row.paymentDate ?? row.createdAt).slice(0, 10); return date >= sessionStart && date <= sessionEnd; });
    ...
    return { invoiceValue, invoiceCount: validInvoices.length, receiptValue, receiptCount: validReceipts.length, totalClientValue, todayCollected, monthlyCollected, yearCollected: sessionCollected, monthlyCollections, clientGrowth, dueClientPayments, aging, pendingClientPayments: dueClientPayments.length, dueTotal: clientDueTotal };
  }, [agreementRows, clientRows, clientPaymentRows, invoiceRows, receiptRows, sessionMode]);

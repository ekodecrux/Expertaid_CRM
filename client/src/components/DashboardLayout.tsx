          ...
          <header className="fixed inset-x-0 top-0 z-50 hidden h-[72px] items-center bg-white/95 shadow-sm backdrop-blur lg:flex">
            ...
            <div className="flex flex-1 items-center justify-end gap-7 px-6 xl:px-8">
              ...
              <div className="relative w-full max-w-[365px]">
                ...
                {globalSearchOpen && globalSearch.trim().length >= 2 && <div className="absolute right-0 top-14 z-[70] w-[min(520px,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_18px_60px_rgba(30,45,80,0.18)]"><div className="border-b border-slate-100 px-4 py-3"><p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-400">Client search</p><p className="mt-1 text-sm text-slate-500">{globalSearchMatches.length ? `${globalSearchMatches.length} matching client${globalSearchMatches.length === 1 ? "" : "s"}` : "No matching client found"}</p></div>{globalSearchMatches.length > 0 ? <div className="max-h-[430px] overflow-y-auto p-2">{globalSearchMatches.map((match) => { const details = buildGlobalClientSearchDetails(match, globalSearchData.products, globalSearchData.plans); return <button key={globalSearchClientKey(match.client)} type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => { setGlobalSearchDetails(details); setGlobalSearchOpen(false); }} className="w-full rounded-xl px-3 py-3 text-left transition hover:bg-[#f5f4ff] focus:bg-[#f5f4ff] focus:outline-none"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-bold text-slate-800">{match.client.clientName}</p><p className="mt-1 text-xs text-slate-500">{match.client.clientId ?? "Client"} · {match.client.session ?? "—"} · {match.client.clientStatus ?? match.client.status ?? "Active"}</p></div><span className="shrink-0 rounded-full bg-violet-100 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-[#4f2ad3]">View details</span></div><div className="mt-3 grid grid-cols-3 gap-2"><span className="rounded-lg bg-slate-50 px-2 py-2"><small className="block text-[9px] uppercase tracking-wide text-slate-400">Current plan</small><strong className="mt-1 block truncate text-xs text-slate-700">{globalSearchDate(match.client.startDate)} – {globalSearchDate(match.client.endDate)}</strong></span><span className="rounded-lg bg-emerald-50 px-2 py-2"><small className="block text-[9px] uppercase tracking-wide text-slate-400">Paid</small><strong className="mt-1 block text-xs text-emerald-700">{globalSearchMoney(details.paid)}</strong></span><span className="rounded-lg bg-rose-50 px-2 py-2"><small className="block text-[9px] uppercase tracking-wide text-slate-400">Pending</small><strong className="mt-1 block text-xs text-rose-700">{globalSearchMoney(details.due)}</strong></span></div><p className="mt-2 text-[11px] text-slate-400">{details.historicalAgreements.length} past plan{details.historicalAgreements.length === 1 ? "" : "s"} · {details.historicalReceipts.length} past receipt{details.historicalReceipts.length === 1 ? "" : "s"}</p></button>; })}</div> : <div className="px-4 py-8 text-center"><Search className="mx-auto h-6 w-6 text-slate-300" /><p className="mt-2 text-sm font-semibold text-slate-700">Try a client name, ID, phone, email, invoice, or receipt number.</p></div>}</div>}
              </div>
...
function GlobalClientDetailsDialog({ details, onClose }: { details: GlobalClientSearchDetails | null; onClose: () => void }) {
  ...
  return <Dialog open={Boolean(details)} onOpenChange={(open) => !open && onClose()}><DialogContent className="max-h-[92vh] w-[min(780px,calc(100vw-2rem))] overflow-y-auto bg-white text-[#172033]"><DialogHeader><DialogTitle className="flex flex-wrap items-center gap-2 font-serif text-2xl"><span>{client.clientName}</span><span className="rounded-full bg-violet-100 px-2.5 py-1 text-xs font-semibold text-[#4f2ad3]">{client.clientId ?? "Client"}</span></DialogTitle><p className="text-sm text-slate-500">Complete client view with current plan, active-cycle collections, products, payment schedule, and historical records.</p></DialogHeader><div className="space-y-4"><section className="rounded-2xl border border-[#ddd8ff] bg-[#faf9ff] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#6b5ce7]">Current plan</p><p className="mt-1 text-lg font-bold text-slate-800">{globalSearchDate(client.startDate)} <span className="font-normal text-slate-400">to</span> {globalSearchDate(client.endDate)}</p><p className="mt-1 text-xs text-slate-500">{client.session ?? details.currentAgreement?.session ?? "—"} · {currentStatus} · {client.instituteType ?? "—"}</p></div><div className="text-right"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400">Grand total</p><p className="mt-1 text-2xl font-bold text-[#4f2ad3]">{globalSearchMoney(details.assigned)}</p></div></div><div className="mt-4 grid gap-3 sm:grid-cols-4"><div className="rounded-xl bg-white p-3"><p className="text-[10px] uppercase tracking-wide text-slate-400">Subtotal</p><p className="mt-1 font-semibold text-slate-800">{globalSearchMoney(client.price ?? client.totalPrice)}</p></div><div className="rounded-xl bg-white p-3"><p className="text-[10px] uppercase tracking-wide text-slate-400">GST</p><p className="mt-1 font-semibold text-slate-800">{globalSearchMoney(client.gstAmount)}</p><p className="text-[10px] capitalize text-slate-400">{client.gstRate ?? 0}% · {client.gstMode ?? "—"}</p></div><div className="rounded-xl bg-emerald-50 p-3"><p className="text-[10px] uppercase tracking-wide text-slate-400">Paid this plan</p><p className="mt-1 font-semibold text-emerald-700">{globalSearchMoney(details.paid)}</p></div><div className="rounded-xl bg-rose-50 p-3"><p className="text-[10px] uppercase tracking-wide text-slate-400">Pending</p><p className="mt-1 font-semibold text-rose-700">{globalSearchMoney(details.due)}</p></div></div></section><section className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">Client and plan details</p><p className="mt-1 text-sm text-slate-500">Identity, ownership, contact, pricing, and current agreement data.</p></div></div><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><div><p className="text-[10px] uppercase tracking-wide text-slate-400">Client owner</p><p className="mt-1 text-sm font-semibold text-slate-700">{client.clientOwnerName ?? "—"}</p></div><div><p className="text-[10px] uppercase tracking-wide text-slate-400">Contact</p><p className="mt-1 text-sm font-semibold text-slate-700">{client.contactNumber ?? "—"}</p></div><div><p className="text-[10px] uppercase tracking-wide text-slate-400">Email</p><p className="mt-1 break-all text-sm font-semibold text-slate-700">{client.email ?? "—"}</p></div><div><p className="text-[10px] uppercase tracking-wide text-slate-400">Pricing</p><p className="mt-1 text-sm font-semibold text-slate-700">{client.pricingMode === "package" ? `Package · ${globalSearchMoney(client.packagePrice)}` : `Per student · ${globalSearchMoney(client.perStudentPrice)}`}</p></div><div><p className="text-[10px] uppercase tracking-wide text-slate-400">Students / employees</p><p className="mt-1 text-sm font-semibold text-slate-700">{Number(client.noOfStudents ?? 0).toLocaleString("en-IN")}</p></div><div><p className="text-[10px] uppercase tracking-wide text-slate-400">Address</p><p className="mt-1 text-sm font-semibold text-slate-700">{client.address ?? "—"}</p></div></div></section>{terms.length > 0 && <section className="rounded-2xl border border-slate-200 p-4"><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">Current payment schedule</p><div className="mt-3 space-y-2">{terms.map((term: any, index: number) => <div key={`${term.id ?? index}-${term.label}`} className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-slate-50 px-3 py-2 text-sm"><span className="font-semibold text-slate-700">{term.label || `Payment ${index + 1}`}</span><span className="text-slate-500">Due {globalSearchDate(term.dueDate)}</span><strong className="text-[#4f2ad3]">{globalSearchMoney(term.amount)}</strong></div>)}</div></section>}{currentProducts.length > 0 && <section className="rounded-2xl border border-slate-200 p-4"><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">Current products and services</p><div className="mt-3 space-y-2">{currentProducts.map((product: any) => <div key={product.id} className="rounded-xl bg-slate-50 px-3 py-2"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-semibold text-slate-700">{product.productName}</span><strong className="text-[#4f2ad3]">{globalSearchMoney(product.totalAmount)}</strong></div><p className="mt-1 text-xs text-slate-500">GST {product.gstRate}% · {product.gstMode} · Paid {globalSearchMoney(product.paidAmount)} · {product.paymentStatus ?? "Pending"}</p></div>)}</div></section>}<section className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">Current-cycle receipts</p><span className="text-xs text-slate-400">{details.currentReceipts.length} receipts · {globalSearchMoney(details.paid)}</span></div>{details.currentReceipts.length > 0 ? <div className="mt-3 space-y-2">{details.currentReceipts.slice(0, 8).map((receipt: any) => <div key={receipt.id ?? receipt.receiptNumber} className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-emerald-50 px-3 py-2 text-sm"><span><strong className="text-slate-700">{receipt.receiptNumber ?? "Receipt"}</strong><span className="ml-2 text-xs text-slate-500">{globalSearchDate(receipt.paymentDate ?? receipt.receiptDate)} · {receipt.paymentMode ?? "—"}</span></span><strong className="text-emerald-700">{globalSearchMoney(receipt.amount ?? receipt.grandTotal)}</strong></div>)}</div> : <p className="mt-3 rounded-xl bg-slate-50 px-3 py-3 text-sm text-slate-500">No receipt collected in the current payment cycle.</p>}</section>{details.currentInvoices.length > 0 && <section className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">Current-cycle invoices</p><span className="text-xs text-slate-400">{details.currentInvoices.length} invoices</span></div><div className="mt-3 space-y-2">{details.currentInvoices.slice(0, 8).map((invoice: any) => <div key={invoice.id ?? invoice.invoiceNumber} className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-blue-50 px-3 py-2 text-sm"><span><strong className="text-slate-700">{invoice.invoiceNumber ?? "Invoice"}</strong><span className="ml-2 text-xs text-slate-500">Due {globalSearchDate(invoice.dueDate ?? invoice.invoiceDate)} · {invoice.status ?? "—"}</span></span><strong className="text-blue-700">{globalSearchMoney(invoice.grandTotal ?? invoice.amount)}</strong></div>)}</div></section>}<section className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">Past plans and payments</p><p className="mt-1 text-xs text-slate-500">Historical renewals and receipts remain visible separately from the current cycle.</p></div><span className="text-xs text-slate-400">{details.historicalAgreements.length} plans · {details.historicalReceipts.length} receipts</span></div>{details.historicalAgreements.length > 0 ? <div className="mt-3 space-y-2">{details.historicalAgreements.map((agreement: any) => <div key={agreement.id} className="rounded-xl bg-slate-50 px-3 py-2 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><span><strong className="text-slate-700">{agreement.session ?? "Historical plan"}</strong><span className="ml-2 text-xs text-slate-500">{globalSearchDate(agreement.startDate)} to {globalSearchDate(agreement.endDate)}</span></span><strong className="text-slate-700">{globalSearchMoney(agreement.totalPrice)}</strong></div><p className="mt-1 text-xs text-slate-500">{agreement.status ?? "—"}{agreement.clientStatus ? ` · ${agreement.clientStatus}` : ""} · GST {agreement.gstRate ?? 0}% {agreement.gstMode ?? ""}</p></div>)}</div> : <p className="mt-3 rounded-xl bg-slate-50 px-3 py-3 text-sm text-slate-500">No historical plan is recorded.</p>}{details.historicalInvoices.length > 0 && <div className="mt-3 border-t border-slate-100 pt-3"><p className="text-xs font-semibold text-slate-600">Historical invoices</p><div className="mt-2 space-y-2">{details.historicalInvoices.slice(0, 8).map((invoice: any) => <div key={invoice.id ?? invoice.invoiceNumber} className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-blue-50 px-3 py-2 text-sm"><span><strong className="text-slate-700">{invoice.invoiceNumber ?? "Invoice"}</strong><span className="ml-2 text-xs text-slate-500">{globalSearchDate(invoice.invoiceDate ?? invoice.dueDate)} · {invoice.status ?? "—"}</span></span><strong className="text-blue-700">{globalSearchMoney(invoice.grandTotal ?? invoice.amount)}</strong></div>)}</div></div>}{details.historicalReceipts.length > 0 && <div className="mt-3 border-t border-slate-100 pt-3"><p className="text-xs font-semibold text-slate-600">Historical receipts</p><div className="mt-2 space-y-2">{details.historicalReceipts.slice(0, 8).map((receipt: any) => <div key={receipt.id ?? receipt.receiptNumber} className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-amber-50 px-3 py-2 text-sm"><span><strong className="text-slate-700">{receipt.receiptNumber ?? "Receipt"}</strong><span className="ml-2 text-xs text-slate-500">{globalSearchDate(receipt.paymentDate ?? receipt.receiptDate)} · {receipt.paymentMode ?? "—"}</span></span><strong className="text-amber-700">{globalSearchMoney(receipt.amount ?? receipt.grandTotal)}</strong></div>)}</div></div>}</section></div></DialogContent></Dialog>;
}

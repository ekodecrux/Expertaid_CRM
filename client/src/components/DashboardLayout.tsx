...
export default function DashboardLayout({
  ...
  const profileUpdate = trpc.profile.update.useMutation({ onSuccess: (saved) => { profile.refetch(); setProfileForm({ ...saved, avatarDataUrl: undefined, avatarPreviewUrl: saved.avatarUrl }); toast.success("Profile settings saved"); setProfileOpen(false); }, onError: (error) => toast.error(error.message) });
  ...
  const [profileForm, setProfileForm] = useState({ displayName: "Workspace administrator", avatarInitials: "AD", avatarColor: "#4b43a8", roleLabel: "Super Admin", department: "Workspace", phone: "", avatarDataUrl: undefined as string | undefined, avatarPreviewUrl: null as string | null });
  useEffect(() => { if (profile.data) setProfileForm({ ...profile.data, avatarDataUrl: undefined, avatarPreviewUrl: profile.data.avatarUrl }); }, [profile.data]);

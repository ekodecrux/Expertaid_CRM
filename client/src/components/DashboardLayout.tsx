...
export default function DashboardLayout({
  ...
  const profileUpdate = trpc.profile.update.useMutation({ onSuccess: (saved) => { profile.refetch(); setProfileForm(saved); toast.success("Profile settings saved"); setProfileOpen(false); }, onError: (error) => toast.error(error.message) });
  ...
  const [profileForm, setProfileForm] = useState({ displayName: "Workspace administrator", avatarInitials: "AD", avatarColor: "#4b43a8", roleLabel: "Super Admin", department: "Workspace", phone: "" });
  useEffect(() => { if (profile.data) setProfileForm(profile.data); }, [profile.data]);
      ...
      <SidebarProvider className="relative flex h-dvh max-h-dvh min-h-0 overflow-hidden bg-[#f8f9fd]" style={{ "--sidebar-width": `${sidebarWidth}px` } as CSSProperties}>
          <header className="fixed inset-x-0 top-0 z-50 hidden h-[72px] items-center bg-white/95 shadow-sm backdrop-blur lg:flex">
            ...
            <div className="flex flex-1 items-center justify-end gap-7 px-6 xl:px-8">
              ...
              <DropdownMenu><DropdownMenuTrigger asChild><button type="button" aria-label="Open admin profile menu" className="flex items-center gap-3 text-left"><Avatar className="h-12 w-12 text-white" style={{ backgroundColor: profile.data?.avatarColor ?? "#4b43a8" }}><AvatarFallback className="text-lg font-semibold text-white" style={{ backgroundColor: profile.data?.avatarColor ?? "#4b43a8" }}>{profile.data?.avatarInitials ?? "AD"}</AvatarFallback></Avatar><span className="hidden min-w-[120px] xl:block"><strong className="block text-sm font-semibold text-slate-800">{profile.data?.displayName ?? user?.name ?? "Admin User"}</strong><small className="block pt-1 text-xs text-slate-500">{profile.data?.roleLabel ?? "Super Admin"}</small></span><ChevronDown className="h-4 w-4 text-slate-500" /></button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-56"><DropdownMenuItem onClick={() => { setProfileForm(profile.data ?? profileForm); setProfileOpen(true); }}><Settings2 className="mr-2 h-4 w-4" />Profile settings</DropdownMenuItem><DropdownMenuItem onClick={() => logout()}><LogOut className="mr-2 h-4 w-4" />Sign out</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
              <Dialog open={profileOpen} onOpenChange={setProfileOpen}><DialogContent className="sm:max-w-[520px]"><DialogHeader><DialogTitle>Profile settings</DialogTitle><DialogDescription>Update the name and profile icon displayed in the workspace header.</DialogDescription></DialogHeader><div className="grid gap-4 py-2"><div className="flex items-center gap-4 rounded-xl bg-slate-50 p-4"><Avatar className="h-16 w-16 text-white" style={{ backgroundColor: profileForm.avatarColor }}><AvatarFallback className="text-xl font-semibold text-white" style={{ backgroundColor: profileForm.avatarColor }}>{profileForm.avatarInitials || "AD"}</AvatarFallback></Avatar><div><p className="text-sm font-semibold text-slate-800">{profileForm.displayName || "Workspace administrator"}</p><p className="text-xs text-slate-500">{profileForm.roleLabel || "Super Admin"}</p></div></div><div className="grid gap-2"><label htmlFor="profile-display-name" className="text-sm font-medium text-slate-700">Display name</label><input id="profile-display-name" value={profileForm.displayName} onChange={(event) => setProfileForm((form) => ({ ...form, displayName: event.target.value }))} className="h-10 rounded-lg border border-slate-200 px-3 text-sm outline-none focus:border-[#5968e8] focus:ring-2 focus:ring-[#5968e8]/10" /></div><div className="grid grid-cols-2 gap-3"><div className="grid gap-2"><label htmlFor="profile-initials" className="text-sm font-medium text-slate-700">Icon initials</label><input id="profile-initials" maxLength={3} value={profileForm.avatarInitials} onChange={(event) => setProfileForm((form) => ({ ...form, avatarInitials: event.target.value.toUpperCase() }))} className="h-10 rounded-lg border border-slate-200 px-3 text-sm uppercase outline-none focus:border-[#5968e8] focus:ring-2 focus:ring-[#5968e8]/10" /></div><div className="grid gap-2"><label htmlFor="profile-color" className="text-sm font-medium text-slate-700">Icon color</label><input id="profile-color" type="color" value={profileForm.avatarColor} onChange={(event) => setProfileForm((form) => ({ ...form, avatarColor: event.target.value }))} className="h-10 w-full rounded-lg border border-slate-200 bg-white p-1" /></div></div><div className="grid grid-cols-2 gap-3"><div className="grid gap-2"><label htmlFor="profile-role" className="text-sm font-medium text-slate-700">Role label</label><input id="profile-role" value={profileForm.roleLabel} onChange={(event) => setProfileForm((form) => ({ ...form, roleLabel: event.target.value }))} className="h-10 rounded-lg border border-slate-200 px-3 text-sm outline-none focus:border-[#5968e8] focus:ring-2 focus:ring-[#5968e8]/10" /></div><div className="grid gap-2"><label htmlFor="profile-department" className="text-sm font-medium text-slate-700">Department</label><input id="profile-department" value={profileForm.department} onChange={(event) => setProfileForm((form) => ({ ...form, department: event.target.value }))} className="h-10 rounded-lg border border-slate-200 px-3 text-sm outline-none focus:border-[#5968e8] focus:ring-2 focus:ring-[#5968e8]/10" /></div></div><div className="grid gap-2"><label htmlFor="profile-phone" className="text-sm font-medium text-slate-700">Phone number</label><input id="profile-phone" value={profileForm.phone} onChange={(event) => setProfileForm((form) => ({ ...form, phone: event.target.value }))} className="h-10 rounded-lg border border-slate-200 px-3 text-sm outline-none focus:border-[#5968e8] focus:ring-2 focus:ring-[#5968e8]/10" /></div></div><DialogFooter><Button type="button" variant="outline" onClick={() => setProfileOpen(false)}>Cancel</Button><Button type="button" disabled={profileUpdate.isPending} onClick={() => profileUpdate.mutate(profileForm)}>{profileUpdate.isPending ? "Saving..." : "Save profile"}</Button></DialogFooter></DialogContent></Dialog>
            </div>
          </header>
          <DashboardLayoutContent setSidebarWidth={setSidebarWidth} branding={companyBranding} sessionSettings={sessionSettings.data} sessionRecords={sessionRecords.data} changeSessionSelection={changeSessionSelection}>
            {children}
